...
var array_= 0;
var wordList=[
    [4,3,5,3],[3,3,3,3],[3,4,5,4],[4,5,4,4],
    [3,4,4,4],[3,3,4,5],[4,4,4,4],[4,2,3,4],
    [3,4,4,5],[3,4,3,4],[5,4,3,5],[3,6,4,4],
    [4,4,4,3],[3,3,5,3],[3,4,4,4],[4,4,3,5],
    [3,3,4,4],[3,5,3,5],[4,4,4,5],[5,3,3,4],
    [3,4,4,4],[3,3,4,5],[3,4,4,4],[3,4,5,3],
    [3,4,4,3],[4,4,4,3],[4,4,5,4],[2,4,5,3]
];
...
$(document).ready(function () {
    ...
    choiceType("0");
    leftLabelImage("00");
    $("#menu-a .left-btn .lbl-a").css("background-image","url(media/00/left-lbl-a-selected.svg)");
    videoType=0;
    $(".main-sub-slider").fadeIn();
    setTimeout(function () {
        choiceVideo2()
    },2000);
    ...
    $(".top-btn").click(function () {
        ...
        if(type==0 && typf==0){
            $(".main-sub-slider").fadeIn();
            $(".main-sub-slider a").removeClass("selected");
            $(".main-sub-slider a:eq(0)").addClass("selected");
            leftLabelImage("0");
            videoType=0;
            choiceVideo2();

        }else {
            $(".main-sub-slider").fadeOut();
            videoType=0;
            choiceVideo();
            leftLabelImage(typf);
        }
    ...
    $("#menu-a .left-btn").click(function () {
        ...
        $(".main-sub-slider a").each(function(){
            if ($(this).hasClass("selected"))
                typA= $(this).attr("type");
        });
        ...
        if(typA=="0" || typA=="01"){
            $("#menu-b").hide();
        }else if(typB=="00" || $(this).attr("type")=="1" ){
            $("#menu-b").show();
            $(".letter-sumb").removeClass("selected");
        }
    ...
    $("#menu-b .letter-sumb").click(function () {
        $(".letter-sumb").removeClass("selected");
        $(this).addClass("selected");
        if($(".swiper-slide:eq(0)").hasClass("selected")){
            videoType=1;
            choiceVideo2()
        }else {
            videoType=1;
    ...
    $(document).on('click', '.main-sub-slider a', function () {
        ...
        leftLabelImage(fileNameB);
        if(fileNameB=='0' || fileNameB=='01'){
            $("#menu-b").fadeOut();
        }else if($("#menu-a .left-btn:eq(0)").hasClass("selected")){
            $("#menu-b").show();
            $(".letter-sumb").removeClass("selected");
        }

        if($(this).attr("type")=="01" || $(this).attr("type")=="0"){
            $("#menu-a .left-btn:eq(2)").hide();
            $("#menu-a .left-btn:eq(3)").hide();
        }else {
            $("#menu-a .left-btn").show();
        }
        videoType=0;
        ...
        switch (videoType){
            case 0:
                ...
                $("#gifLitter").attr("src","media/"+fileNameB+"/"+file+"_"+fileNameA+".gif");
                // $('#LitterVideo').html("");
    ...
    $(document).on('click', '.swiper-slide', function () {
        ...
        if(typC==7 || typC==8 || typC==9 || typC==10 || typC==26){
            $("#menu-a .left-btn:eq(2)").hide();
            $("#menu-a .left-btn:eq(3)").hide();
        }else {
            $("#menu-a .left-btn").show();
...
function leftLabelImage(object){
    if($("#menu-a .left-btn .lbl-a").parent().hasClass("selected")){
        $("#menu-a .left-btn .lbl-a").css("background-image","url(media/"+object+"/left-lbl-a-selected.svg)");
    }else {
        $("#menu-a .left-btn .lbl-a").css("background-image","url(media/"+object+"/left-lbl-a.svg)");
    }

    if($("#menu-a .left-btn .lbl-b").parent().hasClass("selected")){
        $("#menu-a .left-btn .lbl-b").css("background-image","url(media/"+object+"/left-lbl-b-selected.svg)");
    }else {
        $("#menu-a .left-btn .lbl-b").css("background-image","url(media/"+object+"/left-lbl-b.svg)");
    }

    if($("#menu-a .left-btn .lbl-c").parent().hasClass("selected")){
        $("#menu-a .left-btn .lbl-c").css("background-image","url(media/"+object+"/left-lbl-c-selected.svg)");
    }else {
        $("#menu-a .left-btn .lbl-c").css("background-image","url(media/"+object+"/left-lbl-c.svg)");
    }
    if($("#menu-a .left-btn .lbl-d").parent().hasClass("selected")){
        $("#menu-a .left-btn .lbl-d").css("background-image","url(media/"+object+"/left-lbl-d-selected.svg)");
    }else {
        $("#menu-a .left-btn .lbl-d").css("background-image","url(media/"+object+"/left-lbl-d.svg)");
    }
}
...
function choiceVideo() {
    ...
    switch (videoType){
        case 0:
            ...
            /*$("#Litter").find("#gifLitter").remove();
            $('<img src="" id="gifLitter"/>').appendTo("#Litter")*/
    ...
    switch (videoType){
        case 0:
           ...
           // $("#gifLitter").attr("src","images/gif-bg.png");
            ...
            //setTimeout(function(){$("#gifLitter").attr("src","media/"+fileNameB+"/"+file+"_"+fileNameA+".gif");},100)
